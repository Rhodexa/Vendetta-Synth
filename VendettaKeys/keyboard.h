...
void keyboard_scanAndQueue(){
  ...
  for (uint8_t i = 0; i < 8; i++) {
    ...
    key[i +  8].state = ((column_data_lower >> 2) & 0b11);                    
    key[i + 16].state = ((column_data_lower >> 4) & 0b11); 
    key[i + 24].state = ((column_data_lower >> 6) & 0b11);  
    key[i + 32].state = ((column_data_upper     ) & 0b11); // Upper section
    key[i + 40].state = ((column_data_upper >> 2) & 0b11);
    key[i + 48].state = ((column_data_upper >> 4) & 0b11); 
    key[i + 56].state = ((column_data_upper >> 6) & 0b11); 
  }
  ...
  for (uint8_t i = 0; i < N_KEYS; i++) {
    ...
    else { // If not RELEASED nor PRESSED we must be in a HALF_PRESSED state! — The timer always counts when HALF_PRESS'ed, and gets reset in any other case
      key[i].timer++;
    }
  }
}
